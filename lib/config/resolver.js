...
class ConfigResolver {
  ...
  async resolve(key) {
    ...
    if (envValue !== undefined) {
      const localEnvPath = await this.clientRegistry.getStoragePath('env', false);
      return {
        key,
        value: envValue,
    ...
    if (localValue !== undefined) {
      const localJsonPath = await this.clientRegistry.getStoragePath('json', false);
      return {
        key,
        value: localValue,
        source: 'Local Config',
    ...
    if (globalEnvValue !== undefined) {
      const globalEnvPath = await this.clientRegistry.getStoragePath('env', true);
      return {
        key,
    ...
    if (globalValue !== undefined) {
      const globalJsonPath = await this.clientRegistry.getStoragePath('json', true);
      return {
        key,
        value: globalValue,
