...
const { StorageError, FileSystemError, EnvironmentError, ValidationError } = require('../errors');
...
class EnvStorage {
  ...
  /**
   * Get storage path for ENV files
  ...
  getStoragePath(isGlobal = false) {
    ...
    if (isGlobal) {
      ...
      return PathUtils.joinPath(baseDir, serverName, '.env');
    } else {
      // Local: Use current working directory
      return PathUtils.joinPath('.', 'devjoy-digital', serverName, '.env');
    }
  }
  ...
  async read(envPath) {
    try {
      ...
      throw new FileSystemError(
        `Failed to read .env file: ${error.message}`,
        envPath,
        'read',
  ...
  async set(key, value, isGlobal = false) {
    const envPath = this.getStoragePath(isGlobal);
   ...
   */
  async load() {
    // Just load from the library's own storage
    const envPath = this.getStoragePath();
    const envContent = await this.read(envPath);
    
    if (envContent) {
      const vars = this.parse(envContent);
