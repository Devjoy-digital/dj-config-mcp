...
class GitignoreManager {
  ...
  /**
   * Get library's hardcoded config directory for local ENV path
   * @returns {string} Local ENV path
   */
  getLocalEnvPath() {
    const serverName = this.serverName || 'dj-config-mcp';
    return `./devjoy-digital/${serverName}/.env`;
  }
  ...
  async ensure(isGlobal = false) {
    ...
    const localEnvPath = this.getLocalEnvPath();
